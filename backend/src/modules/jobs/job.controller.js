...
exports.createJob = async (req, res) => {
  try {
    ...
    // Check subscription and posting limit
    ...
    if (!subscription.canPostJob()) {
      return res.status(403).json({
        success: false,
        message: `You have reached your job posting limit of ${subscription.plan.jobPostings} jobs. Please upgrade your plan.`,
      });
    }

    // Check featured job limit if this is a featured job
    if (req.body.isFeatured && !subscription.canPostFeaturedJob()) {
      return res.status(403).json({
        success: false,
        message: `You have reached your featured job limit of ${subscription.plan.featuredJobs}.`,
      });
    }
    ...
    if (req.body.isFeatured) {
      await subscription.incrementFeaturedJobs();
    }
...
exports.applyForJob = async (req, res) => {
  try {
    ...
    // Check if already applied
    const alreadyApplied = job.applications.some(
      (app) => app.candidate && app.candidate.toString() === req.user.id
    );
